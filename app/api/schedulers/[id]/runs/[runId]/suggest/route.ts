...
import { suggestAlternativesForSlot } from "@/lib/scheduler-engine";
...
export async function POST(
  ...
  const body = (await request.json().catch(() => ({}))) as { position?: number; limit?: number };
  ...
  try {
    const items = await suggestAlternativesForSlot(id, runId, position!, body.limit);
    return NextResponse.json({ ok: true, items });
  } catch (e) {
    const message = e instanceof Error ? e.message : "Suggesties ophalen mislukt";
